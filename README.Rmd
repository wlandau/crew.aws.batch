...
To get the CloudWatch logs of a job, use the `log()` method. This method writes the log messages with `writeLines()` and invisibly return a `tibble` with timestamped results.
...
Set `tibble = FALSE` to visibly return the `tibble` and omit `writeLines()`.
